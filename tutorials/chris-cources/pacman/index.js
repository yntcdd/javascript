...
for (const [index, row] of map.entries()) {
  for (const [index_, symbol] of row.entries()) {
    switch (symbol) {
      case '-':
        boundaries.push(
          new Boundary({
            position: {
              x: Boundary.width * index_,
      ...
      case '.':
        pellets.push(
          new Pellet({
            position: {
              x: index_ * Boundary.width + Boundary.width / 2,
...
function animate() {
  ...
  for (let index = ghosts.length - 1; 0 <= index; index--) {
  ...
  // touch pellets here
  for (let index = pellets.length - 1; 0 <= index; index--) {
